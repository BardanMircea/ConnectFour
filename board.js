...
function checkLinesForWinner(board) {
  for (let i = 0; i < board.length; i++) {
    ...
    for (let j = 1; j < board[i].length; j++) {
      ...
      if (connectFour === 4) {
        // if we have 4 horizontally connected pieces, return the winner
        return board[i][j];
      }
    }
  }
...
function checkColumnsForWinner(board) {
  for (let i = 0; i < board[0].length; i++) {
    ...
    for (let j = 1; j < board.length; j++) {
      ...
      if (connectFour === 4) {
        // if we have 4 vertically connected pieces, return the winner
        return board[j][i];
      }
    }
  }
...
function checkDiagonalsForWinner(diagonalsArr) {
  for (const line of diagonalsArr) {
    ...
    for (let i = 1; i < line.length; i++) {
      if (line[i] === "-") {
        // if we encounter an unoccupied space, reinitialize the count
        connectFour = 1;
      } else {
        if (line[i - 1] === line[i]) {
          connectFour++; // if we encounter the same consecutive piece, increment the count
      ...
      if (connectFour === 4) {
        // if we have 4 connected pieces, return the winner
        return line[i];
      }
    }
  }
...
function extractRightToLeftDiags(board) {
  ...
  for (let j = 3; j > 0; j--) {
    // we only need the diagonals that are at least 4 positions long
    let tempJ = j;
    let i = 0;
    let row = "";
